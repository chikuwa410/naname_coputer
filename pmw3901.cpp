...
bool PMW3901::readMotionBurst(int16_t *dx, int16_t *dy)
{
    uint8_t rx[12];

    uint8_t cmd = 0x16 & 0x7F;

    cs_low();

    spi_write_blocking(_spi, &cmd, 1);
    spi_read_blocking(_spi, 0x00, rx, 12);
    ...
    // for debug
    // printf("RX:");
    // for(int i=0;i<12;i++) printf("%02X ", rx[i]);
    // printf("\n");

    int16_t raw_dx = (int16_t)((rx[3] << 8) | rx[2]);
    int16_t raw_dy = (int16_t)((rx[5] << 8) | rx[4]);
    uint8_t motion = rx[0];
...
void PMW3901::pmw3901_calculate_velocity_direct(int16_t delta_x, int16_t delta_y,
                                       float altitude, float dt,
                                       float *velocity_x, float *velocity_y)
{
    if (velocity_x == NULL || velocity_y == NULL || dt <= 0.0f) {
        return;
    }
